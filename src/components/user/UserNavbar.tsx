...
const UserNavbar = ({postsCount, followingCount, followersCount}: {postsCount: number, followingCount: number, followersCount: number}) => {
    ...
    return (<nav className="flex flex-col gap-2 justify-between p-4">
        {isViewer ?
            <ButtonOutlined type="button" onClick={undefined} disabled={false} >
                <Link className="text-gradient group-hover:text-white group-hover:reset-text-gradient" to="/profile/me/edit">Edit profile</Link>
            </ButtonOutlined>
        : <ToggleFollowButton isFollowedByViewer={isFollowedByViewer} onClick={onClick} />
        }
        <div className="flex items-center w-full justify-between">
            {links.map(link => (
                <NavLink key={link.label} className="flex flex-col items-center gap-3 max-h-14" to={`${resolver.pathname}/${link.label}`}>
                {({ isActive }) => (
                    <>
                        <span className="capitalize">{link.label}</span>
                        <div className={`flex-center text-${link.color} font-medium text-4xl aspect-square p-3 rounded-full ${isActive ? `bg-${link.color} text-white` : ''}`}>{link.value}</div>
                    </>
                )}
                </NavLink>
            ))}
